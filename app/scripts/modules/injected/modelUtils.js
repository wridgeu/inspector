...
/**
 * Collects the models that reach a root control from above it, which is where the Core's
 * models end up. No version exposes them on the Core facade, and the setter is gone in
 * UI5 2.x, but every version propagates them into the control tree.
...
function _collectPropagatedModels(element, pairs, seen) {
    ...
    // Core models enter the tree at its roots. Further down, a control can keep a copy of
    // a map its parent has since replaced, which would pass a removed model off as a Core
    // one.
    if (parent && parent.isA('sap.ui.core.Element')) {
        return;
    }

    var propagated = element.oPropagatedProperties && element.oPropagatedProperties.oModels;

    // Roots under one UIArea are handed the same map, so reading it once is enough.
...
function _cloneScalar(value, state) {
    ...
    if (type === 'bigint' || type === 'symbol') {
        return {done: true, value: value.toString() + (type === 'bigint' ? 'n' : '')};
    }
...
function _cloneEntries(keys, readValue, state, depth) {
    ...
    keys.slice(0, MAX_ENTRIES).forEach(function (key) {
        ...
        // A throwing getter costs its own entry, not the siblings next to it.
        try {
            value = readValue(key);
        } catch (error) {
        ...
        result[String(key)] = _safeClone(value, state, depth + 1);
    });
...
/**
 * Whether an object has nothing to show, so cutting it off would only hide an empty '{}'.
 * @param {Object} object
 * @returns {boolean}
 * @private
 */
function _isEmptyContainer(object) {
...
function _safeClone(value, state, depth) {
    ...
    // Tagging reads the object too: an Invalid Date or a Proxy can throw right here.
    try {
        var tag = _getObjectTag(value);

        if (tag !== undefined) {
            result = tag;
        } else if (depth > state.maxDepth && !_isEmptyContainer(value)) {
            state.truncated = true;
...
/**
 * Returns the names of the loaded libraries. sap/ui/core/Lib lists them from UI5 1.120,
 * the Core before that.
 * @returns {Array}
 * @private
 */
function _getLibraryNames() {
    var Lib = sap.ui.require('sap/ui/core/Lib');

    return Object.keys(Lib && Lib.all ? Lib.all() : sap.ui.getCore().getLoadedLibraries());
}

/**
 * Whether a component belongs to the framework rather than to an application: a Fiori
 * elements template, a piece of the launchpad.
 * @param {Object} component
 * @param {Array} libraries - names of the loaded libraries
 * @returns {boolean}
 * @private
 */
function _isFrameworkComponent(component, libraries) {
    var type = component.getManifestEntry('/sap.app/type');

    if (type) {
        return type !== 'application';
    }

    // Several launchpad components, and apps without a manifest, declare no type. Those
    // shipped in a loaded library are the framework's.
    var className = component.getMetadata().getName();

    return libraries.some(function (library) {
        return className.indexOf(library + '.') === 0;
    });
}
 ...
 * @param {Array} [libraries] - names of the loaded libraries, not needed for the Core
 * @returns {Object}
 * @private
 */
function _describeOwner(owner, kind, libraries) {
    if (kind === 'Core') {
        return {
            kind: 'Core',
            id: 'sap.ui.core.Core',
    ...
    // UI5 records the owner component only for controls created while a component is being
    // built. Without one - a dialog created in a press handler - the owner counts as the app's.
    var decidingComponent = kind === 'Component' ? owner : ownerComponent;
    ...
    return {
        ...
        framework: !!decidingComponent && _isFrameworkComponent(decidingComponent, libraries)
    };
}
...
function _collectModels() {
    var entries = [];
    var libraries = _getLibraryNames();
    ...
    Object.keys(components).forEach(function (id) {
        ...
        if (component && !_isDestroyed(component) && _hasModels(component)) {
            addOwner(_describeOwner(component, 'Component', libraries), component.oModels);
        }
    });
    ...
    Object.keys(elements).forEach(function (id) {
        ...
        if (_hasModels(element)) {
            addOwner(_describeOwner(element, 'Element', libraries), element.oModels);
        }
...
/**
 * Resolves an id handed out by getModels() against the last scan.
 * @param {string} modelId
 * @returns {Object|undefined} {entry, index, model}, or undefined when the id is stale
 * @private
 */
function _resolveModelId(modelId) {
    var parts = String(modelId).split('-');
    var index = parseInt(parts[2], 10);
    var entry = parseInt(parts[1], 10) === scanCount && lastScan[index];
    var model = entry && entry.ref.deref();

    return model ? {entry: entry, index: index, model: model} : undefined;
}
...
function _createSummary(entry, index, data) {
    ...
    return {
        ...
        name: names.join(', '),
        // Internal once no registration is the app's: each one is on a framework component,
        // or under a name the framework uses on the app's own objects ($ from the UI5
        // libraries, _templPriv* from Fiori elements V2, _pageModel from sap.fe).
        internal: entry.registrations.every(function (registration) {
            return registration.owner.framework || /^(\$|_templPriv|_pageModel$)/.test(registration.name);
        }),
        type: className,
...
module.exports = {
     ...
     * @param {string} [selectedId] - id from the previous scan to carry over
     * @returns {Object} {isSupported: boolean, models: Array, selectedId: string|undefined}
     */
    getModels: function (selectedId) {
        if (!_isSupported()) {
        ...
        try {
            // Resolved before the scan counter moves on, which invalidates the old id.
            var selected = selectedId && _resolveModelId(selectedId);
            ...
            // Matched on the instance: its position in the list can change between scans.
            var selectedIndex = selected ? entries.map(function (entry) {
                return entry.model;
            }).indexOf(selected.model) : -1;

            return {
                isSupported: true,
                models: models,
                selectedId: selectedIndex === -1 ? undefined : models[selectedIndex].id
            };
        } catch (error) {
            console.warn(error);
            lastScan = [];
    ...
    getModelDetails: function (modelId) {
        var resolved = _resolveModelId(modelId);
        ...
        return {
            summary: _createSummary({model: model, registrations: entry.registrations}, resolved.index, data),
