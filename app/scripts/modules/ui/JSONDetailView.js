...
function JSONDetailView(containerId, options) {
    ...
    this.oEditorDOM.classList.add('hidden');
    ...
    this.oEditorAltMessageDOM.textContent = this.sEmptyMessage;
    ...
    // The json mode's worker only annotates syntax errors, which serialized data cannot
    // have, and the extension does not ship it.
    this.oEditor.getSession().setUseWorker(false);
    ...
    // The marker is editor text, not a button, so Enter on its line is the keyboard path.
...
JSONDetailView.prototype.update = function (vData, sAltMessage) {
    ...
    if (!bHasData) {
        this.mRowIds = {};
        this.oEditorAltMessageDOM.textContent = sAltMessage || this.sEmptyMessage;
        return;
    }
